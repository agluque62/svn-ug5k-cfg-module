...
void Uv5kiGwCfgWebApp::stCb_dev(struct mg_connection *conn, string user, web_response *resp)
{
	...
	else if (string(conn->request_method)=="POST")
	{
		...
		RETURN_IERROR_RESP(resp, webData_line("Error en Formato de trama. Pocos par�metros.").JSerialize());
		}
	}
	RETURN_NOT_IMPLEMENTED_RESP(resp);
}
...
void Uv5kiGwCfgWebApp::SynchronizeConfigIfApplicable() {
	...
	if (P_WORKING_CONFIG->DualCpu()) {
		...
		//si no es mode2, me sincronizo si está aislada (aislada significa que no tengo comunicacion con centralizada)
		if ( ((redan_mode2) && (P_CFG_PROC->GetStdLocalConfig() != slcAislado) )
